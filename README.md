...
- **Interactive Step-by-Step Customization:** Choose exactly what gets installed with clear explanations and prompts for each of the 14 phases (Laravel, Docker, AI CLIs, etc.), or run unattended with `-y` / `--yes`.
...
```bash
# Interactive mode (prompts and explains each phase)
curl -sSL desktop.setupvibe.dev | bash

# Automatic mode (installs all phases without confirmation)
